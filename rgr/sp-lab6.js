...
const expression = `var a, b, n : byte;  a[n]:=15*10;`.toLowerCase()
...
let ifAmount = 0;
// const typesHIerarchy = ["extended","double","real","single","longint","word","integer","shortint","byte"]
// console.log(typesHIerarchy.reverse())
...
const types = {
  ...
  "single" : {
    "maxValue" : 2147483647000000,
    "minValue" : 0,
    "withDot" : true
  },
  "real" : {
    "maxValue" : 214748364700000000000,
    "minValue" : 0,
    "withDot" : true
  }
}

const typesHierarchy = [ 'byte',
'shortint',
'integer',
'word',
'longint',
...
//! VAR a,b : INTEGER;
  ...
  if (lexemsTable[index] === "T_ASSIGNMENT") {
    ...
    if (
      ...
      ( 
        lexemsTable[index - 1] !== "T_RIGHT_BRACKET" &&
        lexemsTable[findBracketLeftPair(index - 1)-1] !== "T_VARIABLE"
      ) 
    ) {
      errorOccured = true;
      console.log(
  ...
  if (!errorOccured && index > lexemsTable.length) {
    ...
    performOperations(programStartIndex);
    buildTree();
  }
};
  ...
  if ( variable === "true" || variable === "false" ){
    ...
    if(variable.indexOf(".") !== -1){
      ...
      if(+variable > 0 && +variable < 255){
        console.log("byte")
        return "byte"
      }else if(+variable > -128 && +variable < 127){
        console.log("shortint")
        return "shortint"
      }else if(+variable > 0 && +variable < 65535){
        console.log("word")
        return "word"
      }else if(+variable > -32768 && +variable < 32768){
        console.log("integer")
        return "integer"
      }else if(+variable > -2147483648 && +variable < 2147483647){
        return "longint"
      }
    }
...
const performOperations = (startIndex) => {
  let areThereOperations = lexemsTable.includes("T_ASSIGNMENT", startIndex);
  let temporaryNumericValue = "";
  let temproraryStringValue = "";
  let temproraryBooleanValue = "";
  let semicolunmIndex ;
  let operationIndex = lexemsTable.indexOf("T_ASSIGNMENT", startIndex);
  let index = operationIndex+1;

  while(areThereOperations){ 
    semicolunmIndex = lexemsTable.indexOf("T_SEMICOLUMN", index);
    
    
    // if(lexemsTable[operationIndex-1] === "T_RIGHT_BRACKET"){
    //   while(index < semicolunmIndex){
      
    //   }
    // }
    if(
      lexemsTable[operationIndex-1] === "T_VARIABLE" || 
      lexemsTable[operationIndex-1] === "T_RIGHT_BRACKET"
      ){
      let variableToAssign;
      if(lexemsTable[operationIndex-1] === "T_RIGHT_BRACKET"){
        console.log("zalupa")
       variableToAssign=declaredVariables[expressionsTable[operationIndex-4]]
      }
      else{
        variableToAssign=declaredVariables[expressionsTable[operationIndex-1]]
      }
      if(variableToAssign !== undefined){
      while(index < semicolunmIndex){
          if(
            lexemsTable[index] === "T_VARIABLE"
          ){
            const variable = declaredVariables[expressionsTable[index]];
            if(variable !== undefined){
              // if(
              //   identifyType(variableToAssign.value) !== "boolean" &&
              //   identifyType(variableToAssign.value) !== "char"){
              //     temporaryVault += 
              //   }
              if(variable.class === "array"){
                if(index+2 < variable.length){
                  temporaryNumericValue += variable.value[index]
                  index+=4
                }else{
                  throw new Error(`
                  \nError at index ${expressionsIndexes[index+2]}, out of "${expressionsTable[index]}" length\n\n
                `)
                }
                
              }else{
                temporaryNumericValue += variable.value;
                index++;
              }
              
              console.log(temporaryNumericValue);
            }else{
              throw new Error(`
              \nError at index ${expressionsIndexes[index]}, variable "${expressionsTable[index]}" isn't declared\n\n
            `)
            }
            // console.log(eval("100*12"))
//             
// 
// 
// 
...
// 
            ...
            // if(valueVaer > type.maxvalue
            // if(typesHierarchy.indexOf(variableToAssign.type) > typesHierarchy.indexOf(variable.type)){
            //   variableToAssign.value = variable.value;
            // }else {
            //   throw new Error(`
            //   \nError at index ${expressionsIndexes[index+1]}, variable "${expressionsTable[index+1]}" has wrong type\n\n
            // `)
            // }

          }
          // else if (
          //   expressionsTable[operationIndex + 1] === "-"
          // ){

          // }
          else if(lexemsTable[index] === "T_BINARY_OPERATOR"){
            temporaryNumericValue += expressionsTable[index];
            index++;
            console.log(temporaryNumericValue);
          }else if(
            lexemsTable[index] === "T_INTEGER" ||
            lexemsTable[index] === "T_FLOAT"
          ){
            temporaryNumericValue += expressionsTable[index];
            console.log(temporaryNumericValue);
            index++;
          ...
          // else{
          //   if(typesHierarchy.indexOf(variableToAssign.type) > typesHierarchy.indexOf(identifyType(expressionsTable[operationIndex + 1]))){
          //     // console.log(indenifyType(expressionsTable[operationIndex + 1]))
          //     variableToAssign.value = +expressionsTable[operationIndex + 1];
          //   }else {
          //     throw new Error(`
          //     \nError at index ${expressionsIndexes[operationIndex+1]}, variable "${expressionsTable[operationIndex-1]}" has wrong type\n\n
          //   `)
          //   }
        ...
        result = eval(temporaryNumericValue);

        if(
          types[variableToAssign.type].maxValue > result &&
          types[variableToAssign.type].minValue < result){
            if(variableToAssign.class === "array"){
              [operationIndex-2] < variable.length
              temporaryNumericValue += variable.value[index]
            }else{
              variableToAssign.value = result;
            }
        ...
        console.log(variableToAssign.value);
      }else{
          throw new Error(`
          \nError at index ${expressionsIndexes[operationIndex-1]}, variable "${expressionsTable[operationIndex-1]}" isn't declared\n\n
        `)
        }
      
    }

    areThereOperations = lexemsTable.includes("T_ASSIGNMENT", index);
  }
