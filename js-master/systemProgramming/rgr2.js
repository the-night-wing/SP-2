...
const pascal = process.argv[2];
let areThereAsign = pascal.match(":=");
let c = pascal;
while (areThereAsign) {
  c = c.replace(":=", "=");
  areThereAsign = c.match(":=");
}
// console.log({ c });
let areThereFloats = c.match("float");
while (areThereFloats) {
  c = c.replace("float", "double");
  areThereFloats = c.match("float");
}
let areThereWords = c.match("word");
while (areThereWords) {
  c = c.replace("word", "short");
  areThereWords = c.match("word");
}
c = c.replace("byte", "short");
c = c.replace("var ", "");
...
let columnIndex = c.indexOf(":");
while (columnIndex !== -1) {
  arrayLength = 0;
  identifiersString = c.substring(0, columnIndex);
  identifiers = identifiersString.split(",");
  identifiers = identifiers.map(e => e.trim());

  if (c.substr(columnIndex + 1, 5) === "array") {
    if (c.charAt(columnIndex + 12).match(/[1234567890]/)) {
      arrayLength = c.substr(columnIndex + 11, 2);
    } else {
      arrayLength = c.charAt(columnIndex + 11);
    }
    currentType = c
      .substring(
        c.indexOf("of", columnIndex + 1) + 2,
        c.indexOf(";", columnIndex + 1)
      )
      .trim();
  } else {
    currentType = c.substring(columnIndex + 1, c.indexOf(";", columnIndex + 1));
  }

  if (arrayLength !== 0) {
    identifiers = identifiers.map(e => `${e}[${arrayLength}]`);
  }
  if (previousType === currentType) {
    // variableDeclaration += `${currentType} ${identifiers.join(",")};`;
    variableDeclaration =
      variableDeclaration.substring(0, variableDeclaration.length - 1) +
      `,${identifiers.join(",")};`;
  } else {
    variableDeclaration += `${currentType} ${identifiers.join(",")};`;
  }
  // c = c.substring(0, columnIndex) + c.substring(columnIndex + 1, c.length + 1);
  c = c.substring(c.indexOf(";", columnIndex + 1) + 1, c.length).trim();
  columnIndex = c.indexOf(":");
  previousType = currentType;
  // console.log({ previousType });
  // console.log({ currentType });
}
// console.log({ identifiers });
// console.log({ identifiersString });
// console.log({ arrayLength });
// console.log({ currentType });
// console.log({ variableDeclaration });
// console.log({ c });
// c = c.indexOf(":");
// console.log(columnIndex + " smth");

c = variableDeclaration + c;
// console.log({ c });
...
const main = () => {
        ...
        if (!after[i].Type.includes(before[0].Type)) {
          if (after[i].Type !== "short" || before[i].Type !== "double") {
            if (before[i].Type !== "double[4]") {
              ...
              // console.log("\n\n")
              //       errorLogging(`ERROR: Wrong expression.
              // You can't assign '${after[i].token}' with 'float' type
              // to variable '${before[i].token}' with 'word' type.`);
  ...
  to variable '${before[i].token}' with 'word' type.`);
    ...
    if (exprs[0].includes("[-")) {
      console.log(
        `ERROR: Index of a array should be >= 0 and <= ${arrayIndex}`
      );
      process.exit(0);
    }
    ...
    for (const el in hash) {
      ...
      try {
        ...
        if (!value) {
          console.log("\n\n");
          console.log("ERROR: Wrong placed brackets ", hash[el]);
          console.log("\n\n");
          process.exit(0);
        }
    ...
    console.log("\n\n");
    // throw new Error("Wrong brackets at index : " + errorIndex );
    console.log({ results });
  ...
  vars = vars.filter((item, index, arr) => {
    if (arr.indexOf(item) !== index) {
      console.log("\n\n");
      console.log(`ERROR: Variable "${item}" has already been declared`);
      console.log("\n\n");
      process.exit(1);
    }
  });
...
// console.log("\n\n");
// console.log(`{ results: [ { name: 'b', value: -20 } ] }`);
